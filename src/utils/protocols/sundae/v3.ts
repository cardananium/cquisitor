...
// Only fields we surface in the UI are
...
function parseAssetTriple(d: PD): AssetAmount {
  // V3 orders encode this as a flat List<policy, name, amount>; Stableswap
  // orders encode the SAME triple as a nested Constr0[policy, name, amount].
  // Accept both.
  const fields = isList(d) ? asList(d) : asConstr(d).fields;
  if (fields.length !== 3) throw new Error(`expected (policy, name, amount) triple`);
  return {
    policyId: asBytes(fields[0]),
    assetName: asBytes(fields[1]),
    amount: asInt(fields[2]),
  };
}
...
function parseBound(d: PD): IntervalBound {
  // NegativeInfinity = ctor 0; Finite(t) = ctor 1; PositiveInfinity = ctor 2.
  const c = asConstr(d);
...
// SundaeSwap pools across all three protocol generations. V1 lives in ./v1
// (its own decoder file); V3 + Stableswap are defined above.
export type SundaePoolDatum = V3PoolDatum | StableswapPoolDatum | V1PoolDatum;
